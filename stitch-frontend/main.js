...
function App() {
    ...
    const [phase, setPhase] = useState('entering-name');
        ...
        const initConnection = async () => {
            try {
                ...
                client.on(ServerMessageType.MATCH_CREATED, (msg) => {
                    ...
                    setLogs((p) => [...p, `MATCH CREATED: Code ${msg.payload?.code}`]);
                });
                ...
                client.on(ServerMessageType.ERROR, (msg) => {
                    ...
                    setLogs((p) => [...p, `SERVER ERROR: ${msg.payload?.message}`]);
                    setIsLoading(false);
                });
    ...
    const handleNameSubmit = (codename) => {
    ...
    // };
    const handleInitiateOperation = (frequency) => {
    ...
    const handleLinkToNetwork = (frequency) => {
    ...
    return (_jsxs("div", { style: { width: '100%', height: '100vh' }, children: [phase === 'entering-name' && (_jsx(CodenameAuthorizationTerminal, { operativeCodename: playerName, sector: "07-B", latitude: "38.9072\u00B0 N", longitude: "77.0369\u00B0 W", threatLevel: isConnected ? 'Normal' : 'High', terminalLog: logs, onEstablish: handleNameSubmit, onInputChange: handleInputChange, loading: isLoading })), phase === 'deployment' && (_jsx(MissionDeploymentHub, { operativeName: playerName ? `OPERATIVE_${playerName.toUpperCase()}` : 'OPERATIVE_01', sector: "BERLIN_VOID", turnCycle: "05/12", location: "Berlin", coverLevel: 92, networkStatus: "Secure", intelUpdate: "Intercepting encrypted traffic from Sector 7...", threatLevel: "Local authorities increasing patrol frequency.", environment: "Heavy rain. Visual range reduced to 500m.", latitude: "52.5200\u00B0 N", longitude: "13.4050\u00B0 E", logs: logs, onInitiateOperation: handleInitiateOperation, onLinkToNetwork: handleLinkToNetwork, onTerminateLink: () => {
                ...
                }, loading: isLoading })), phase === 'playing' && (_jsx(SurveillanceCommandCenterGlobal, { operativeName: playerName ? `OPERATIVE_${playerName.toUpperCase()}` : 'OPERATIVE_01', sector: "BERLIN_VOID", location: "Berlin", coverLevel: 92, turnCycle: "05/12", latitude: "52.5200\u00B0 N", longitude: "13.4050\u00B0 E", cities: [

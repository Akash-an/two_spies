...
const HowToPlayOverlay: React.FC<HowToPlayOverlayProps> = ({ onClose }) => {
  ...
  const [showOriginalRules, setShowOriginalRules] = useState(false);

  return (
    <div className="howto-overlay">
      <div className="howto-card">
        <button className="howto-close" onClick={onClose}>✕</button>
        <div className="howto-header-row">
          <h2 className="howto-title">How to Play</h2>
          <button 
            className={`howto-toggle-btn ${showOriginalRules ? 'active' : ''}`}
            onClick={() => setShowOriginalRules(!showOriginalRules)}
          >
            <span className="material-symbols-outlined">
              {showOriginalRules ? 'description' : 'image'}
            </span>
            <span>{showOriginalRules ? 'VIEW GUIDE' : 'VIEW ORIGINAL RULES'}</span>
          </button>
        </div>
        ...
        {showOriginalRules ? (
          <div className="howto-image-container">
            <img 
              src="/assets/original_rules.png" 
              alt="Original Game Rules" 
              className="howto-original-img"
            />
          </div>
        ) : (
          <>
